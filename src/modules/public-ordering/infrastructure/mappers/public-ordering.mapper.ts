...
export function mapPublicTableSession(payload: Record<string, unknown>): PublicTableSession {
  ...
  return {
    ...
    businessLogoUrl: String(business?.logoUrl ?? payload.logoUrl ?? ""),
    businessPrimaryColor: String(business?.primaryColor ?? payload.primaryColor ?? ""),
    businessThemeKey: String(business?.themeKey ?? payload.themeKey ?? ""),
